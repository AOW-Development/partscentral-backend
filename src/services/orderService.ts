import { PrismaClient, OrderStatus, OrderSource, PaymentStatus, AddressType } from '@prisma/client';

const prisma = new PrismaClient();

interface CreateOrderPayload {
  billingInfo: any; // Use the actual type from frontend store
  shippingInfo: any; // Use the actual type from frontend store
  customerInfo: any; // Use the actual type from frontend store
  cartItems: any[]; // Use the actual type from frontend store
  paymentInfo: any; // Use the actual type from frontend store
  totalAmount: number;
  subtotal: number;
}
...
export const createOrder = async (payload: CreateOrderPayload): Promise<any> => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const newAddress = await tx.address.create({
      data: {
        addressType: AddressType.RESIDENTIAL, // Assuming residential for now, can be dynamic
    ...
    const order = await tx.order.create({
      data: {
        ...
        status: OrderStatus.PENDING, // Will update to PAID after payment processing
        subtotal: subtotal,
        totalAmount: totalAmount,
        billingSnapshot: billingInfo,
        shippingSnapshot: shippingInfo,
        addressId: newAddress.id,
        addressType: AddressType.RESIDENTIAL, // Assuming residential for now
    ...
    await tx.payment.create({
      data: {
        ...
        status: PaymentStatus.SUCCEEDED, // Assuming payment is successful at this point
        paidAt: new Date(),
